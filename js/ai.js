...
// Get best move based on score gained in next several moves
AI.prototype.getMoveMaxScoreAfterKMoves = function(grid, k) {
  ...
  for (var direction = 0; direction < 4; direction++) {
    ...
    //calculates expectation of score
    else {
...
AI.prototype.getHeuristic = function(grid) {
  ...
  grid.eachCell(function(x, y, cell) {
    if (!cell) {
      //add large score for empty cells
      score += 4096;
    }
    else {
      //score += cell.value;
      ...
      //add score if largest value is in the border
...
// nneonneo's answer from stackoverflow
